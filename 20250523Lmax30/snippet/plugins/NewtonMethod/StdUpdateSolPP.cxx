...
void StdUpdateSolPP::execute()
{
  ...
  CFreal maxvA = max(2.0*Bmagmax*Bref / pow((_rhoBC * rhoref * mu0), 0.5), 3.0e6);   
  //---- modified on 2026.05.08<<------------------------
  //>>---- Mark 20260215------------------
	  ...
	  if (SubSystemStatusStack::getActive()->getNbIter()>=0){
		...
		//	  cur_state[7] -= m_alpha[7] * dU(iState, 7, nbEqs);
		  ...
		  */
		  ...
		  if (r < 1.1 + 0.002){
			  /*
			  CFreal Bx = cur_state[4];
			  CFreal By = cur_state[5];
			  CFreal Bz = cur_state[6];
			  CFreal Vx = cur_state[1];
			  CFreal Vy = cur_state[2];
			  CFreal Vz = cur_state[3];
			  CFreal VdotB = Bx*Vx + By*Vy + Bz*Vz;
			  CFreal VprojectB = VdotB / Bmag;
			  cur_state[1] = VprojectB*Bx / Bmag;
			  cur_state[2] = VprojectB*By / Bmag;
			  cur_state[3] = VprojectB*Bz / Bmag;
			  */
