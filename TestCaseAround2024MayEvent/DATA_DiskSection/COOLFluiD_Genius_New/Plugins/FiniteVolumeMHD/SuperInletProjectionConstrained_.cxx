...
#include "FiniteVolumeMHD/SuperInletProjectionConstrained.hh"
...
//////////////////////////////////////////////////////////////////////////////
MethodCommandProvider<SuperInletProjectionConstrained, CellCenterFVMData, FiniteVolumeMHDModule>  
...
SuperInletProjectionConstrained::SuperInletProjectionConstrained(const std::string& name) :
  SuperInlet(name),
  m_mapGeoToTrs(),
  //>> mark 2025.05.25 refered to Tiko
  //m_mapTrs2Twall(),
  //<<
  m_BfromFile(false)
{
...
{
//>> mark 2025.05.27 by Hp
  Common::SafePtr<SubSystemStatus> subSysStatus = SubSystemStatusStack::getActive();
  CFreal relax_fac=(subSysStatus->getNbIter()>2000)? 1.0:((double)subSysStatus->getNbIter())/2000.0;
  //CFreal relax_fac=(subSysStatus->getNbIter()>2000)? 1.0:1.0;
//<<
  ...
  if (m_initialSolutionMap.size() > 0) {
    ...
    for (CFuint i = 0; i < nbVars; ++i) {
      ...
      //CFLog(INFO, "SuperInletProjection::setGhostState() => [" << varID << "] => " << (*initialValues)[idx] << " | " << (*innerState)[varID] << "\n");
	}
  }
  //CFLog(INFO, "m_initialSolutionMap.size()="<<m_initialSolutionMap.size()<<", m_BfromFile="<<m_BfromFile<<"\n");
...
//<<
  if (m_initialSolutionMap.size() > 0 && m_BfromFile == true) {
    ...
    BrFromFile /= (double)nbNodesInFace;
     //CFreal maxBr = std::max(maxBr, BrFromFile);
     //CFLog(INFO, "Br from file #### varID = " << varID << ", maxBr = " << maxBr << "\n");
  }
  //CFLog(INFO,"BrFromFile="<<BrFromFile<< "\n");
...
//>> mark 2025.05.27 by Hp  
  if (SubSystemStatusStack::getActive()->getDT() < 1e-3) {
  CFreal Br_PF=xBoundary_dimless/rBoundary_dimless*B_PFSS_dimless[0] + yBoundary_dimless/rBoundary_dimless*B_PFSS_dimless[1] + zBoundary_dimless/rBoundary_dimless*B_PFSS_dimless[2];
  BrFromFile=Br_PF+(BrFromFile-Br_PF)*relax_fac;
  }
//<<
...
// CFLog(INFO, "m_BfromFile[" << m_BfromFile << "], Br from File vs BrBoundary_dimless = " << BrFromFile << " ? " << BrBoundary_dimless << " \n"); 
  ...
  if(Vmag>0.0){
  CFreal dist = (Vmag * vref - Vmagmax*1.0)/2.0e0;
  ...
  CFreal maxvA = 2.0e6; //1.0e6; // mark 2024.12.05
