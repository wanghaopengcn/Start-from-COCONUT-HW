    ...
    if((trs->hasTag("inner")) && (trs->hasTag("cell")))
    {
      ...
      for (CFuint iType = 0; iType < elementType->size(); ++iType)
      {
        ...
        if (nbCellsInType > 0)
        {
		  ...
		  // Mark transfer the time unit from code unit to physical hour
          CFreal solutiontime = subSysStatus->getCurrentTimeDim() > 0 ? subSysStatus->getCurrentTimeDim()*1447.2/3600.0 : subSysStatus->getNbIter();
          ...
          if (getMethodData().getAppendAuxData())
            fout << ", AUXDATA CPU=\"" << PE::GetPE().GetRank(nsp) << "\""
                 ...
                 << ", AUXDATA PhysTime=\"" << subSysStatus->getCurrentTimeDim()*1447.2/3600.0 << "\""
                 << flush;
          fout << ", VARLOCATION=( ";
                      if (dim!=1)
                        fout   << "[" << 1 << "-" << dim << "]=NODAL";
