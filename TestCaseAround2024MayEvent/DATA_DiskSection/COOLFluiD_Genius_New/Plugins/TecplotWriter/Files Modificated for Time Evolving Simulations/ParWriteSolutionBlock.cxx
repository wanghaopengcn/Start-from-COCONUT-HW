	...
	if (iVar == totNbVars-1) {
	  if (tt.nodesOffset[iType].second != maxpos) {
	    CFLog(ERROR, "tt.nodesOffset[iType].second (" << tt.nodesOffset[iType].second 
		  << ") != maxpos (" << maxpos << ")" << "\n");
		 //>> mark 2024.05.02 commented by Hp 
					    ...
					    const bool isBoundary) 
{
  ...
  *fout << "ZONE "
  ...
  fout->precision(14);  
  fout->setf(ios::scientific,ios::floatfield);
  // mark transfer hour from code unite to physical unite.
  const CFreal timeDim = SubSystemStatusStack::getActive()->getCurrentTimeDim()*1447.2/3600.0;
