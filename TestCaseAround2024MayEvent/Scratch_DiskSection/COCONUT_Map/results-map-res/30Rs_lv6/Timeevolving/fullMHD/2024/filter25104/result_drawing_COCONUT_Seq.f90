...
Program output
  ...
  INTEGER(4),PARAMETER::N_th_p=90, N_ph_p=N_th_p*2, Rindex=72
  !INTEGER(4),PARAMETER::Nodstart=1,Nodend=192150,Cellstart=1,Cellend=378880,R_IndMax=74
  !>>for 30Rs_lvl6
  INTEGER(4),PARAMETER::Nodstart=1,Nodend=768150,Cellstart=1,Cellend=1515520,R_IndMax=74
  !>>for BCindependent6
  ...
  real*8 :: FR_cent,Long0,Long,Longitude_shift,halfLength,rad_temp,minute_drop,save_first
  ...
  integer :: mmddhh,year,hh_increased,seconds,second,minutes,minute,hours,hour,days,months
  ...
  logical :: interface_for_euhforia,update,Mesh_30Rs_lvl5,Mesh_BCindependent6,Mesh_30Rs_lvl6
  ...
  Time_sequence= .false. !.true. !
  writereformated_dat = .false.
  !define "Time_sequence= .false." before seting "SortorNot = .true."
  !.true. means rearanging information for transfering to structured grid
  SortorNot = .true.  !.false. !
  !-------------------------------------
  ...
  pannels= .false. !.true. !
  interface_for_euhforia = .true. !.false. !
  update= .true. !.false.
  rdial_profile= .false.
  writeBxyzonly= .false. !.true. !
  write_CFmesh = .true. ! .false. !
  write_r=.true.
  if(Time_sequence .eqv. .true.)then 
    write_r=.false.
	write_CFmesh = .false.
  end if
  Mesh_30Rs_lvl5= .false.
  Mesh_30Rs_lvl6= .true.
  ...
  cadence=1.0
  time_step=100 !16 
  Seq_Start=428 !800 !185 !278 !96 !0 !76 !60 !84 !735 !82 !581 !117 !380  
  save_first=0.0 
  if(Time_sequence .eqv. .false.)then
      Seq_End=Seq_Start
  else
      Seq_End=1146 !606 !150 !100 !800 !379 !650
  end if
  time0=0.0
  time=time0
  ...
  index_=0  
  timescale=100 !1 !
  write(*,*)"index_=",index_,"timescale=",timescale
  ...
  time=time0*timescale+Seq_*cadence+save_first*timescale
  eps=1.d-3
...
!!>>Step1: Reformat COCONUT data to structured data
  ...
  !filename='./supplement/corona-time_'//ctime//'.plt'
...
write(*,*)'Start Writing CFcase of t = ',ctime,' hour'
 ...
 if(Mesh_BCindependent6 .eqv. .true.)then        
  ...
  write(27,*)'!NB_NODES_PER_TYPE 6'
 else if(Mesh_30Rs_lvl6 .eqv. .true.)then
  write(27,*)'!NB_NODES 768150 0'
  write(27,*)'!NB_STATES 1515520 0'
  write(27,*)'!NB_ELEM 1515520'
  write(27,*)'!NB_ELEM_TYPES 1'
  write(27,*)'!GEOM_POLYORDER 1'
  write(27,*)'!SOL_POLYORDER 0'
  write(27,*)'!ELEM_TYPES Prism'
  write(27,*)'!NB_ELEM_PER_TYPE 1515520'
  write(27,*)'!NB_NODES_PER_TYPE 6' 
 end if
...
!!      write(*,*) "phi=",PHI/pi*180.,"theta=",THETA/pi*180.  
    ...
    !goto 112  
      ...
      !Synoptic maps at 20Rs--------------
      usph=0.d0
	  if(justnearest .eqv. .false.)then
          call sph_interp(68,radiu(68),Rusph_interp(1:17,0:N_th_p+1,0:N_ph_p+1))
      else
          call sph(68,Rusph_interp(1:17,0:N_th_p+1,0:N_ph_p+1)) 
	  end if
	  usph(1:17,0:N_th_p+1,0:N_ph_p+1)=Rusph_interp(1:17,0:N_th_p+1,0:N_ph_p+1)
      filename_out='./Pannel/Synoptic_maps20Rs'//ctimeo//'.plt'
      open(1,file=filename_out)
      write(1,*) 'TITLE="2.5sph DATA"'
      write(1,*) 'VARIABLES = "Heliolongitude (deg)","Heliolatitude (deg)","Vr (Km/s)","Vth (Km/s)","Vph (Km/s)",&
...
113	  if(interface_for_euhforia .eqv. .true.)then 
	  ...
	  usph(1:17,0:N_th_p+1,0:N_ph_p+1)=Rusph_interp(1:17,0:N_th_p+1,0:N_ph_p+1)
      filename_out='/lustre1/scratch/357/vsc35794/euhforia_events/plt/21d5Rs'//ctimeo//'.plt'
      open(1,file=filename_out)
      write(1,*) 'TITLE="2.5sph DATA"'
      write(1,*) 'VARIABLES = "Heliolongitude (deg)","Heliolatitude (deg)","Vr (Km/s)","Vth (Km/s)","Vph (Km/s)",&
	        "N (10<sup>3</sup> cm<sup>-3</sup>)","Br (G)","Bth (G)","Bph (G)","T (10<sup>5</sup> K)","p (Pa)",&
	  ...
	  minutes=int(((seconds-second)+0.001)/60.d0)
	  minute= mod(minutes,60) !mod(int(seconds/60),60)
	  hh_increased=hh_increased+ int(((minutes-minute)+0.001)/60.d0) !int(int(seconds/60)/60)
	  ...
	  filename_out='/lustre1/scratch/357/vsc35794/euhforia_events/dat/solar_wind_boundary_'//ctime_2//'.dat'
	  open(1,file=filename_out)
	  write(1,'(A)')"Time:"
	  ...
	  write(1,'(A)')"vr"
	  !do i1= 1, N_th_P ! mark 2025.03.13
	   do j1 = 1, N_ph_P
		do i1= 1, N_th_P ! mark 2025.03.13
		  vr =usph(12,i1,j1)*Vs
		  line = ''
...
112    print *, "Hello, over"
    ...
    subroutine coordinate_xyz_rtp(r,theta,phi,x,y,z)!球坐标与直角坐标的转换
        implicit double precision(a-h,o-z)
        pi = 4.0*datan(1.d0)
  ...
  real*8  :: CCOF(2,2)
    ...
    subroutine initial_reconstruction_B_cell(U,aB,wB,vB,agspt_weightB_point,Uxyz)          !rbf片片间磁场失量插值      
    implicit none
...
!        end do        
    ...
    subroutine recon_cellrbf(W_,a,w,v,agspt_weight7_point,Wxyz)                           !rbf片片间标量插值 
    implicit none
...
!!          if(j1==int(N_th_P/3) .and. k1==int(N_ph_p/3))write(*,*)'detmin=',detmin,'i1=',i1,'j1=',j1,'k1=',k1
         ...
         do j1=0, N_ph_p+1
		  ...
		  if(SortorNot .eqv. .false.)then
            ...
            DO jj1=1,PerCellLayer
			    !---------->>mark 2025.03.17----------------
			    !do ii1=max(i-1,0),min(i+1,R_IndMax) 
                do ii1=max(i-2,1),min(i+2,R_IndMax-2)
				!------------mark 2025.03.17<<--------------
                  index=ii1*PerCellLayer+jj1
                  x_yin=Cell_Pos(1,index)
                  y_yin=Cell_Pos(2,index)
  ...
  do i=0,i_cur
	...
	if(i==0)hour=mod(N_CARRINGTON_NOs,100)
