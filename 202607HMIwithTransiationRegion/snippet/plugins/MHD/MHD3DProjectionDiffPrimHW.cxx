					       ...
					       const CFreal& radius)
{
  ...
  */
   ...
   const CFreal epsilon = 1.0e-8; //1.0e-16; // you should not add eps if Bnorm is > 0.
   //>> Mark 20260214 -----------------------------
   ...
   CFreal qF_ = Bx*dTdx + By*dTdy + Bz*dTdz;
   CFreal sgnq = 0.0;
   if (qF_ > 0.0){
	   sgnq = 1.0;
   }
   ...
   CFreal C_sound = std::sqrt(1.667*p_temp*P0 / (rho_temp * rho0));
   //C_sound = max(C_sound, Va/10.0);
   //C_sound = max(C_sound, Vnorm);
   //C_sound = min(C_sound, V0/10.0);
   CFreal qSound = -1.5*rho_temp * rho0*std::pow(C_sound, 3.0)*(qF_x*normal[XX] + qF_y*normal[YY] + qF_z*normal[ZZ]);
   qSound = qSound / 3.0;
   //--->> corrected on July 11, 2026 --------------
   //CFreal smooth_factor = max(0.0, min(1.0 - 0.5*std::abs(qFlux_1 / qSound), 1.0));
   //CFreal smooth_factor = max(0.0, min(1.0 - 0.5*(qFlux_1 / qSound), 1.0));
   //CFreal smooth_factor = max(0.0, min(1.0 - 0.5*(qFlux_1 / qSound), 1.0));
   
   CFreal smooth_factor_1 = max(0.0, min(1.0 - 0.5*(qFlux_1 / qSound), 1.0));
   CFreal Rho_threshold = 1.0e1;
   CFreal dist = 1.0e3*(rho_temp - Rho_threshold) / Rho_threshold;
   CFreal smooth_factor_2 = 0.5 + 0.5 * std::tanh(dist * 3.141592654);
   CFreal smooth_factor = smooth_factor_1*smooth_factor_2 + 1.0*(1.0 - smooth_factor_2);
   ...
   //smooth_factor = max(0.0, min(1.0 - 0.5*(qFlux_2 / qSound), 1.0));
   //----- corrected on July 11, 2026 <<--------------
   //smooth_factor = 1.0;
