  ...
  socket_divBCellCenter("divBCellCenter")
{
  ...
  _TR = 0;
  setParameter("TransitionRegion", &_TR);
  //-- added by Hp Wang <<----------
...
void MHDConsACAHWSourceTerm::defineConfigOptions(Config::OptionList& options)
{
  ...
  options.addConfigOption< CFint >("TransitionRegion", "Switch on scaling of tansition region heating terms.");
  //-- added by Hp Wang <<----------
					 ...
					 RealMatrix& jacobian)
{
  ...
  const CFreal mu_cor =1.27; // 1.0; // Molecular weight in the corona 
 ...
 // const CFreal l0 = 6.9e8;
  ...
  CFreal rho_ = (*currState)[0];
  CFreal Rho_HCthreshold = 100.0;
  ...
  //CFreal Qh2 = Qh4H_const*min(Bnorm, B_thresh)*1e-7 / 1e-6 / 1e-4;
  //----->> added on 2026.04.27 by Hp Wang---------------------------
  //CFreal dist = max((r - RSun*1.1) / (RSun),0.0);
  CFreal dist = max((r - RSun*1.0) / (RSun), 0.0)*100.0;
  //CFreal dist = r / RSun;
  CFreal smooth_factor = 1.0 + 2.0 * std::tanh(dist * 3.141592654);
  //CFreal smooth_factor = 1.0 + 1.0 * std::tanh(dist * 3.141592654);
  if (_TR == 0){
	  smooth_factor = 1.0;
  }
  Qh2 = Qh2*smooth_factor;
  CFreal T_threshold = 200000.0;
  //Qh2 = Qh2 / std::pow((1 + std::pow(T / T_threshold, -10)), 0.25);
  //------- added on 2026.04.27 by Hp Wang<<---------------------------
  // Qh3 is the exponential heating function which is uniform around the Sun.
  CFreal Qh3 = 1e-4 * std::exp(-(r - RSun)/(0.4*RSun)) * ( 1e-7 / 1e-6);
//  CFreal Qh3 = 4.9e-5 * std::exp(-(r - RSun)/(0.7*RSun)) * ( 1e-7 / 1e-6);
  CFreal Qh_extra = 5.0e-7 * std::exp(-(r - RSun)/(1*RSun)) * ( 1e-7 / 1e-6);
...
//  CFreal Br_c = 1.0e-4;
  ...
  //-->> added by Hp Wang----------
	  ...
	  //CFreal Rho_HCthreshold = 4.0;
	  CFreal Q_rad = 0.0;
	  CFreal RadCur_Cur = 0.0;
	  double Coefi1;
	  double Coefi2;
	  CFreal Temp;
	  CFreal N_thred = density;
	  dist = 1e1*(density - Rho_HCthreshold*rhoRef) / min(density, Rho_HCthreshold*rhoRef);
	  smooth_factor = 0.5 + 0.5 * std::tanh(dist * 3.141592654);
	  //if (r / RSun<1.2){
	  smooth_factor = 1.0;
	  //N_thred = density*(1.0 - smooth_factor) + Rho_HCthreshold*rhoRef*smooth_factor;
	  //}
	  if ((T < pow(10, 4.0))) {
		  Q_rad = 0.0;
	  }
	  else if (T > pow(10, 9.0)){
		  Q_rad = std::pow(1e-6*N_thred / (mu_cor*mH), 2)*RadCur[m_sampleTem - 1] * (1e-7 / 1e-6);
	  }
	  else{
		  for (CFuint i = 0; i < m_sampleTem - 1; ++i){
			  if ((T - temperature[i])*(temperature[i + 1] - T) >= 0.){
				  ...
				  Q_rad = std::pow(1e-6*N_thred / (mu_cor*mH), 2)*RadCur_Cur * (1e-7 / 1e-6);
			  }
		  }
	  }
  ...
  //--------- Mark 2026.05.26 --------------
  //if (T <  T_threshold && T > 0.0) {
  if (r / RSun<10.0){
	  Q_rad_athay = Q_rad_athay / std::pow((1 + std::pow(T / T_threshold, -10)), 0.25);  // *std::pow(T/1000000, 5.);  //*std::pow((1+std::pow(T/1000000,-10)),0.5); //*std::pow(T/500000, 4.); //*(T-20000)*(T-20000)/T/T;
	  //-->> added by Hp Wang----------
	  Q_rad = Q_rad / std::pow((1 + std::pow(T / T_threshold, -10)), 0.25);
	  //-- added by Hp Wang <<----------
  }
  else{
	  Q_rad_athay = Q_rad_athay*(1.0 / std::pow((1 + std::pow(T / T_threshold, -10)), 0.25)*(1.0 - min((r / RSun - 10.0) / 2.5, 1.0)) + min((r / RSun - 10.0) / 2.5, 1.0));
	  Q_rad = Q_rad*(1.0 / std::pow((1 + std::pow(T / T_threshold, -10)), 0.25)*(1.0 - min((r / RSun - 10.0) / 2.5, 1.0)) + min((r / RSun - 10.0) / 2.5, 1.0));
  }
  //}
...
//      const CFreal R2_m = 1./2.*((vxdim/vax+1)*dvaxdx+(vydim/vay+1)*dvaydy+(vzdim/vaz+1)*dvazdz);
      ...
      lambda_perp = 0.01*RSun * std::sqrt(1e-4/Bnorm); // Should be 1e-4 
...
//      std::cout << "thirs term " << third_term_plus* (*currState)[9] << "  " <<endl;
    ...
    if (_RadiativeLossTerm == 1) {
		// Comment for Quentin
		// In the first run keep this zero; when you rerun then comment this so that the real flux is calculated
		//Q_rad_athay = 0.0;
		//Q_rad = 0.0;
		dist = max((r - RSun*1.1) / (RSun), 0.0);
		smooth_factor = max(std::tanh(dist * 3.141592654), 0.0);		
		smooth_factor = 1.0;
		if (_CHIANTI9 == 0){
			source[7] -= Q_rad_athay*nondimconst*volumes[elementID] * smooth_factor;
			socket_radiativeloss.getDataHandle()[elementID] = Q_rad_athay* smooth_factor;
		}
		//-->> added by Hp Wang----------
		else if (_CHIANTI9 == 1){
			source[7] -= Q_rad*nondimconst*volumes[elementID] * smooth_factor;
			socket_radiativeloss.getDataHandle()[elementID] = Q_rad* smooth_factor;
		}
		//-- added by Hp Wang <<----------
    }
